    ...
    <>
      ...
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 h-16 backdrop-blur-xl border-b transition-colors duration-300 ${
          scrolled ? 'bg-background/92 border-border' : 'bg-transparent border-transparent'
        }`}
            ...
            >
              <ul className="flex flex-col gap-1 p-5">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[
                    { label: 'Instagram', href: 'https://www.instagram.com/aiclub_dau/', emoji: '📸' },
                    { label: 'Discord', href: 'https://discord.gg/yB3Huet5', emoji: '🎮' },
                    { label: 'GitHub', href: 'https://github.com/ai-club-dau', emoji: '💻' },
                    { label: 'LinkedIn', href: 'https://www.linkedin.com/company/ai-club-dau/', emoji: '💼' },
                  ].map(link => (
                    <a
                      key={link.label}
                      href={link.href}
